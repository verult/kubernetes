...
var map_PersistentVolumeSpec = map[string]string{
	...
	"accessModes":                   "Describes the volume's access modes. Possible values are: * ReadWriteOnce - can be mounted read/write mode to exactly 1 node. * ReadOnlyMany - can be mounted in read-only mode to many nodes. * ReadWriteMany - can be mounted in read/write mode to many nodes. More info: https://kubernetes.io/docs/concepts/storage/persistent-volumes#access-modes",
	"claimRef":                      "Part of a bi-directional binding between PersistentVolume and PersistentVolumeClaim. Expected to be non-nil when bound. claim.VolumeName is the authoritative bind between PV and PVC. When set to non-nil value, PVC.Spec.Selector of the referenced PVC is ignored, i.e. labels of this PV do not need to match PVC selector. More info: https://kubernetes.io/docs/concepts/storage/persistent-volumes#binding",
